...
export function usePlayerDecks() {
  ...
  //
  // previousName, when given, is deleted in this SAME upsert — a rename is
  // one atomic write, not a save-under-the-new-name followed by a separate
  // delete-the-old-name. Doing those as two calls would leave a real window
  // where both names exist (or, worse, where a failed second call leaves an
  // orphaned duplicate deck behind).
  const saveDeck = useCallback(async (name, deck, previousName) => {
