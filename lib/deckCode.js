...
//
// `releasedElements` (a Set of element names) is what stops a code minted
// somewhere Ice/Magnetic/Black Flame are visible from opening in this
// builder full of held-back content — the same content-leak FocusSim's own
// deckCode.js guards against with unreleasedCards(), ported here as a
// decode-time refusal instead of a separate post-decode check, since
// there's no legitimate reason to ever construct the deck object first.
export function decodeDeck(input, cardsById, releasedElements) {
  ...
  const leaderCard = cardsById[leader];
  if (!leaderCard || leaderCard.type !== "leader") {
    return { error: "That code names a Leader that does not exist." };
  }
  if (!releasedElements.has(leaderCard.element)) {
    return { error: "That code names a Leader that isn't released yet." };
  }
  ...
  for (let i = 0; i < entryCount; i++) {
    ...
    if (!card) return { error: `That code contains a card that does not exist (${id}).` };
    if (!releasedElements.has(card.element)) {
      return { error: "That code contains a card that isn't released yet." };
    }
